...
export default function Wais() {
  ...
  const handleSubtestChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Additional safeguard: only allow numeric strings
  ...
  const handlePacienteSelect = (paciente: Paciente) => {
    ...
    // Parse keys from schema if available, otherwise just use names
    // Note: backend names are snake_case but interface is camelCase
    setDatosGenerales(prev => ({
      ...prev,
      nombresPaciente: paciente.nombresApellidos,
      fechaNacimiento: paciente.fechaNacimiento ? new Date(paciente.fechaNacimiento) : null
    }));
    setIsSearchModalOpen(false);

    // Auto calculate age if birth date is present
    if (paciente.fechaNacimiento) {
      setTimeout(() => calcularEdadAuto(new Date(paciente.fechaNacimiento!)), 100);
    }
  };

  const calcularEdadAuto = (fechaNacimiento: Date) => {
    const fechaTest = new Date(datosGenerales.fechaTest);
    fechaTest.setHours(0, 0, 0, 0);
    fechaNacimiento.setHours(0, 0, 0, 0);
    ...
    if (dias < 0) {
      meses--;
      const ultimoMes = new Date(fechaTest.getFullYear(), fechaTest.getMonth(), 0);
      dias += ultimoMes.getDate();
    }
  ...
  const calcularResultados = async () => {
    ...
    for (const key of Object.keys(subtests)) {
      const valRaw = subtests[key];
      const val = valRaw === "" ? null : Number(valRaw);
      if (val !== null) {
    ...
    try {
      ...
      console.error("Error en cálculo WAIS:", error);
      setErrores([
  ...
  const generarPDF = () => {
    ...
    const indices = [
      { label: "ICV", value: resultado.resultados.icv?.indice ?? 0 },
      { label: "IRP", value: resultado.resultados.irp?.indice ?? 0 },
      { label: "IMT", value: resultado.resultados.imt?.indice ?? 0 },
      { label: "IVP", value: resultado.resultados.ivp?.indice ?? 0 },
      { label: "CIT", value: resultado.resultados.cit?.indice ?? 0 },
    ];
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                // Allow only letters, spaces and common Spanish accents
                if (val !== "" && !/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]*$/.test(val)) return;
                setDatosGenerales((p) => ({
                  ...p,
                  nombresExaminador: val,
                }));
              }}
            />
          </div>
      ...
      <ComponentCard title="Subtests – Puntuación Directa">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {Object.entries(subtests).map(([id, value]) => {
            const resultData = resultado?.subtests?.[id];
            return (
                ...
                <div className="space-y-1">
                  ...
                  {resultado && (
                    <div className="flex flex-col text-xs mt-1">
                      {resultData?.scaled !== undefined && (
                        <span className="text-blue-600 dark:text-blue-400 font-medium">
                          Escalar: {resultData.scaled ?? "N/A"}
                        </span>
                      )}
                      {resultData?.error && (
      ...
      {resultado && (
        <ComponentCard title="Resultados">
          ...
          <div className="p-4 bg-white dark:bg-transparent">
            <WaisProfileChart
              icv={resultado.resultados.icv?.indice ?? 0}
              irp={resultado.resultados.irp?.indice ?? 0}
              imt={resultado.resultados.imt?.indice ?? 0}
              ivp={resultado.resultados.ivp?.indice ?? 0}
              cit={resultado.resultados.cit?.indice ?? 0}
            />
          </div>
        </ComponentCard>
      )}
